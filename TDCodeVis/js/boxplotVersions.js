...
d3.csv("../js/read/boxplotVersions.csv", function(error, dados) {
  ...
  for (var [key, dados] of Object.entries(data)) {
    ...
    obj["color"] = selectColor(severity);
  ...
  var yScale = d3.scaleLinear()
    ...
    .domain([min-1,max]) //Poderia ser 0 também.
    .range([height, 0]);
  ...
  for (var i = boxPlotData.length - 1; i >= 0; i--) {
    for (var j = boxPlotData[i].outliers.length - 1; j >= 0; j--) {
      ...
      g.selectAll(".whiskers")
        ...
        .style("stroke", selectColor(severity)) //Borda dos outliers na cor de acordo com a severidade do grupo.
