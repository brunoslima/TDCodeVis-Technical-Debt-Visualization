...
d3.csv("../js/read/bubblechartAmount.csv", function(error, data){
    ...
    bolhas.append("circle")
        ...
        .on("click", function(d) {
            window.location="../view/treemapissues.php?type=1-issue="+d.title;
          })
        .style("fill", function(d) { return selecionarCores(d["severity"]); }) //cor de acordo com a severidade da issue.
        .append("title")
            .text(function(d){ return getDescription(d["title"],d["amount"],d["severity"],d["description"]);}); //Descrição da issue.
...
function getDescription(title, amount, severity, description){
    var rotulo = "";
    var severityTxt = "";
